...
static void symbol_handler(zbar_decoder_t *dcode)
{
    ...
    for (sym = iscn->syms->head; sym; sym = sym->next)
        ...
        {
            sym->quality++;
            fprintf(stderr, "data: %s sym->data: %s sym->quality: %d\n", data, sym->data, sym->quality);
...
int zbar_scan_image(zbar_image_scanner_t *iscn, zbar_image_t *img)
{
    ...
    if (iscn->syms->nsyms > 0)
    {
        const zbar_symbol_t *sym = iscn->syms->head;

        // 画像データを取得
        const unsigned char *image_data = (const unsigned char *)img->data;
        unsigned int img_width = img->width;
        unsigned int img_height = img->height;

        while (sym)
        {
            if (sym->type > ZBAR_PARTIAL && sym->npts > 0)
            {
                // バーコードの座標を計算
                int minX = img_width, maxX = 0;
                int minY = img_height, maxY = 0;

                for (unsigned int i = 0; i < sym->npts; i++)
                {
                    if (sym->pts[i].x < minX)
                        minX = sym->pts[i].x;
                    if (sym->pts[i].x > maxX)
                ...
                // 画像範囲チェック
                if (centerY >= 0 && centerY < img_height &&
                    minX >= 0 && maxX < img_width)
                {

                    // 輝度データを収集
                    int width = maxX - minX + 1;
                    unsigned char *brightness = malloc(width);

                    for (int i = 0; i < width; i++)
                    {
                        unsigned int idx = centerY * img_width + (minX + i);
                        brightness[i] = image_data[idx];
                    }
                    ...
                    for (int i = 0; i < width; i++)
                    {
                        if (brightness[i] < min_val)
                            min_val = brightness[i];
                        if (brightness[i] > max_val)
                            max_val = brightness[i];
                        sum += brightness[i];
                    }
                    unsigned char avg_val = sum / width;

                    // 閾値を計算（平均値を使用）
                    unsigned char threshold = (min_val + max_val) / 2;

                    fprintf(stderr, "\n========================================\n");
                    fprintf(stderr, "FINAL BARCODE RESULT\n");
                    fprintf(stderr, "========================================\n");
                    fprintf(stderr, "Type: %s\n", zbar_get_symbol_name(sym->type));
                    fprintf(stderr, "Data: %s\n", sym->data);
                    fprintf(stderr, "Position: [%d,%d - %d,%d]\n", minX, minY, maxX, maxY);
                    fprintf(stderr, "Center Line: Y=%d\n", centerY);
                    fprintf(stderr, "----------------------------------------\n");
                    fprintf(stderr, "BRIGHTNESS DATA (%d pixels):\n", width);
                    fprintf(stderr, "----------------------------------------\n");

                    // // 輝度データを出力
                    // for (int i = 0; i < width; i++) {
                    //     fprintf(stderr, "%3d", brightness[i]);
                    //     if ((i + 1) % 10 == 0) {
                    //         fprintf(stderr, "\n");
                    //     } else if (i < width - 1) {
                    //         fprintf(stderr, ", ");
                    ...
                    // fprintf(stderr, "  Min: %3d (darkest)\n", min_val);
                    // fprintf(stderr, "  Max: %3d (brightest)\n", max_val);
                    // fprintf(stderr, "  Avg: %3d\n", avg_val);
                    // fprintf(stderr, "  Threshold: %3d\n", threshold);
                    // fprintf(stderr, "  Contrast: %3d\n", max_val - min_val);

                    // // ★バーとスペースの幅を計算
                    // fprintf(stderr, "========================================\n");
                    // fprintf(stderr, "BAR/SPACE WIDTH ANALYSIS\n");
                    // fprintf(stderr, "========================================\n");
                    // fprintf(stderr, "Threshold: %d (values below = BAR, above = SPACE)\n", threshold);
                    // fprintf(stderr, "----------------------------------------\n");
                    ...
                    int space_count = 0;
                    int current_width = 1;
                    int is_bar = (brightness[0] < threshold) ? 1 : 0; // 最初がバーかスペースか

                    // fprintf(stderr, "Pattern (B=Bar, S=Space):\n");
                    // fprintf(stderr, "----------------------------------------\n");

                    // for (int i = 1; i < width; i++) {
                    //     int current_is_bar = (brightness[i] < threshold) ? 1 : 0;

                    //     if (current_is_bar == is_bar) {
                    //         // 同じ状態が続く
                    //         current_width++;
                    //     } else {
                    //         // 状態が変わった（バー→スペース または スペース→バー）
                    //         if (is_bar) {
                    //             fprintf(stderr, "B:%3d  ", current_width);
                    //             bar_count++;
                    //         } else {
                    //             fprintf(stderr, "S:%3d  ", current_width);
                    //             space_count++;
                    //         }

                    //         if ((bar_count + space_count) % 5 == 0) {
                    //             fprintf(stderr, "\n");
                    //         }

                    //         current_width = 1;
                    //         is_bar = current_is_bar;
                    //     }
                    // }
                    ...
                    fprintf(stderr, "  Total Bars: %d\n", bar_count);
                    fprintf(stderr, "  Total Spaces: %d\n", space_count);
                    fprintf(stderr, "  Total Elements: %d\n", bar_count + space_count);
                    fprintf(stderr, "========================================\n\n");
